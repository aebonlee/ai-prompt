...
27. **상단 메뉴 클릭 시 대표 페이지 이동** - 드롭다운 메뉴에 path 추가, 데스크톱 hover로 하위 메뉴 표시 + 클릭 시 대표 페이지 이동, 모바일 클릭 토글 유지

28. **메인 아이콘 컬러셋 연동** - var(--primary) → var(--primary-blue)로 변경, 컬러셋 변경 시 아이콘 색상 실시간 반영

29. **푸터 리뉴얼 (db-study 패턴 적용)** - GitHub Repository 링크 삭제, 연락처(이메일/전화/카카오톡/영업시간) 추가, Family Site 드롭다운 추가, 하단 사업자 정보(대표이사/사업자번호/통신판매번호/출판사번호) 추가, hover 색상 블루(#93C5FD) 적용
...
- `d79ca8a` docs: DEVLOG/CHANGELOG 업데이트 (드롭다운 수정, og:title 변경 반영)
- `d89ae25` feat: 상단 메뉴 클릭 시 대표 페이지 이동 + hover로 하위 메뉴 표시
- `9a20dd8` fix: 메인 페이지 아이콘 색상이 컬러셋 변경에 연동되도록 수정
