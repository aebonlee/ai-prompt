...
export default function WorkbookWrite() {
    ...
    return (
      <section className="section">
        <div className="container">
          <div className="lecture-empty">로그인이 필요합니다.</div>
          <div style={{ textAlign: 'center' }}>
            <Link to="/login" className="lecture-action-btn primary">로그인</Link>
          </div>
        </div>
      </section>
    )
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!form.week_number || !form.title.trim()) {
      setError('주차와 제목을 입력해주세요.')
      return
    }
    ...
    const fileUrl = form.file_url.trim()
      ? (form.file_url.trim().startsWith('http') ? form.file_url.trim() : `/pdf/${form.file_url.trim()}`)
      : ''
          ...
          <form className="lecture-write-form" onSubmit={handleSubmit}>
            ...
            <div className="lecture-form-group">
              ...
              <div className="lecture-form-file-input">
                <span className="lecture-form-file-prefix">/pdf/</span>
                <input
                  type="text"
                  value={form.file_url}
                  onChange={(e) => handleChange('file_url', e.target.value)}
                  placeholder="파일명.pdf (또는 전체 URL)"
                />
              </div>
            </div>
