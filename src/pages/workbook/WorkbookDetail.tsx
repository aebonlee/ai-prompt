...
export default function WorkbookDetail() {
  ...
  const loadWorkbook = async () => {
    setLoading(true)
    const { data } = await getWorkbook(id)
    setWorkbook(data)
    if (data) {
      incrementWorkbookViews(id)
    }
    setLoading(false)
  }
    ...
    return (
      <section className="section">
        <div className="container">
          <div className="lecture-empty">워크북을 찾을 수 없습니다.</div>
          ...
          <div className="lecture-detail">
            <div className="lecture-detail-header">
              ...
              <div className="lecture-detail-meta">
                <span>{workbook.author_name}</span>
                <span>{new Date(workbook.created_at).toLocaleDateString('ko-KR')}</span>
                <span>조회 {workbook.views || 0}</span>
              </div>
            </div>
            ...
            {workbook.file_url && (
              <div className="lecture-file-section">
                {user ? (
                  <a
                    href={workbook.file_url.startsWith('http') ? workbook.file_url : `${import.meta.env.BASE_URL}${workbook.file_url.replace(/^\//, '')}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="lecture-action-btn primary"
                  >
                    파일 보기 / 다운로드
                  </a>
                ) : (
            ...
            <div className="lecture-detail-actions">
              ...
              {isOwner && (
                <>
                  <Link to={`/workbook/edit/${id}`} className="lecture-action-btn primary">
                    수정
                  </Link>
                  <button className="lecture-action-btn danger" onClick={handleDelete}>
                    삭제
                  </button>
                </>
              )}
