...
const categories = ['전체', '자유', '질문', '팁 공유', '프롬프트 공유']
const PAGE_SIZE = 10
...
export default function CommunityList() {
  ...
  return (
    <>
      <SEOHead title="커뮤니티" description="AI 프롬프트 엔지니어링 커뮤니티 - 질문, 팁 공유, 프롬프트 공유" />

      <section className="page-header">
        <div className="container">
          <h1>커뮤니티</h1>
          <p>프롬프트 엔지니어링에 대해 질문하고 팁을 공유하세요</p>
        </div>
      </section>
          ...
          <div className="lesson-body" style={{ maxWidth: '900px' }}>
            <div className="community-toolbar">
              <div className="community-category-filter">
                {categories.map(cat => (
                  <button
                    key={cat}
                    className={`community-category-btn${category === cat ? ' active' : ''}`}
            ...
            {loading ? (
              ...
              <div className="community-empty">
                <p>게시글이 없습니다.</p>
                {user && <Link to="/community/write" className="community-write-btn" style={{ marginTop: '1rem' }}>첫 글을 작성해보세요</Link>}
              </div>
            ) : (
            ...
            {!user && (
              <div className="community-login-prompt" style={{ marginTop: '1.5rem' }}>
